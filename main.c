...
int main(void) {
    ...
    for(int i = 0; i<n ; i++){
        for(int j = 0; j<m; j++){
            ...
            start.y = j;
            if(map[i][j] == 'c'){
                for(int k = 0; k < villNum; k++){
                    end = villCordination[k];
                    road(map, n, m, start, end, 1);
                }
            }
            else if(map[i][j] == 'b'){
                for(int k = 0; k < villNum; k++){
                    end = villCordination[k];
                    road(map, n, m, start, end, 2);
                }
            }
        }
    }
        ...
        for(int i = 0; i<n; i++) {
            for (int j = 0; j < m; j++) {
                if (map[i][j] == 'c') {
                    ...
                    DrawRectangle(startX + i * cellSize, startY + j * cellSize, cellSize - 2, cellSize - 2, BROWN);
                }
                else if(map[i][j] == 'R'){
                    DrawRectangle(startX + i * cellSize, startY + j * cellSize, cellSize - 2, cellSize - 2, PURPLE);
                }
                else {
