...
int main(void) {
    ...
    // Main game loop
    int start=villNum;
    while (!WindowShouldClose() && start!=0)    // Detect window close button or ESC key
        {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                if (map[i][j] == 'c') {
                    ...
                    if (map[i][j] == 1) {
                        DrawRectangle(startX + i * cellSize, startY + j * cellSize, cellSize - 2,
                                      cellSize - 2, level1);
                    } else if (map[i][j] == 2) {
                        DrawRectangle(startX + i * cellSize, startY + j * cellSize, cellSize - 2,
                                      cellSize - 2, level2);
        ...
        int Round=0;
            if (IsKeyPressed(KEY_ONE)) {
                ...
                if(k1.gold>=1) {
                    buyFood(k1);
                    Round++;
                    DrawText("Round", 850, 20, 80, DARKBLUE);
                }
                else{
                    DrawText("Enter Again",850, 20,80,DARKBLUE);
                }
            }
